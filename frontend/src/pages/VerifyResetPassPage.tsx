...
export default function VerifyResetPassPage() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
	  ...
	  try {
		const response = await fetch(`${import.meta.env.VITE_USER_SERVICE}/api/auth/verify/reset-password`, {
		  ...
		  body: JSON.stringify({ oldPassword, password: newPassword, confirmPassword: confirmNewPassword }),
		});
	...
	return (
	  <div className=" bg-background w-full flex items-center justify-center text-text dark:bg-backgroundDark mt-10 dark:text-textDark h-screen">
		<div className="bg-white border-1 text-center p-10 border-gray-300 w-[450px] h-auto rounded-2xl">
		  ...
		  <form onSubmit={handleSubmit} className="flex flex-col gap-5 my-8">
			<input
			  ...
			  className="w-full py-4 px-3 border bg-white border-gray-200 rounded-lg"
