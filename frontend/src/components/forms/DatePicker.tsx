...
export default function DatePickerWithFullWidthButtons() {
    ...
    const renderCalendar = useRef(() => {}); // Initialize as empty function
  
    useEffect(() => {
      renderCalendar.current();
    }, [currentDate, isCalendarOpen, renderCalendar]); // Added renderCalendar to dependency array
    ...
    useEffect(() => {
      renderCalendar.current = () => {
        ...
        daysContainer.innerHTML = ""; // Property 'innerHTML' does not exist on type 'never'.
  
        for (let i = 0; i < firstDayOfMonth; i++) {
          const emptyDiv = document.createElement("div");
          daysContainer.appendChild(emptyDiv); // Property 'appendChild' does not exist on type 'never'.
        }
        ...
        for (let i = 1; i <= daysInMonth; i++) {
          ...
          dayDiv.addEventListener("click", () => {
            const selectedDateValue = `<span class="math-inline">${month + 1}/</span>{i}/${year}`;
            setSelectedDate(selectedDateValue);
            if (daysContainer) {
              daysContainer
                .querySelectorAll("div")
                .forEach((d) => d.classList.remove("bg-primary", "text-white", "dark:text-white")); // Argument of type 'string' is not assignable to parameter of type 'SetStateAction<null>'.
              dayDiv.classList.add("bg-primary", "text-white", "dark:text-white");
            }
          });
          daysContainer.appendChild(dayDiv); // Property 'appendChild' does not exist on type 'never'.
        }
      };
    }, [currentDate]); // Removed isCalendarOpen as it doesn't directly affect rendering of days
